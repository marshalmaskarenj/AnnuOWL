"""The first of two related components, that generates the OVNI first ring (Well-being/Circadian) Geometry. The second component -- Vert2SGeoB -- merges these geometries and its Output connects to Custom Preview Geometry.
---
...
self.Name = "SCALE_Vert2SGeoA"
self.NickName = 'Vert2SGeoA'
self.Message = 'AnnuOWL | Vert2SGeoA\nAUG_15_2023'
...
for i in range (0,4,1):
    ...
    anglePo=ang[3]
    angleMo=ang[4]
    p0=rs.CreatePoint(aa[0]+0.8*Rad*math.cos(angleMo),aa[1]+0.8*Rad*math.sin(angleMo),aa[2])
    p1=rs.CreatePoint(aa[0]+0.8*Rad*math.cos(angle),aa[1]+0.8*Rad*math.sin(angle),aa[2])
    p2=rs.CreatePoint(aa[0]+0.8*Rad*math.cos(anglePo),aa[1]+0.8*Rad*math.sin(anglePo),aa[2])
    p3=rs.CreatePoint(aa[0]+1.47*Rad*math.cos(angleM),aa[1]+1.47*Rad*math.sin(angleM),aa[2])
    p4=rs.CreatePoint(aa[0]+1.47*Rad*math.cos(angle),aa[1]+1.47*Rad*math.sin(angle),aa[2])
    p5=rs.CreatePoint(aa[0]+1.47*Rad*math.cos(angleP),aa[1]+1.47*Rad*math.sin(angleP),aa[2])
