"""The first of two related components, that generates the OVNI outer ring (View Quality) Geometry. The second component -- Vert4SGeoB -- merges these geometries and its Output connects to Custom Preview Geometry.
---
...
self.Name = "SCALE_Vert4SGeoA"
self.NickName = 'Vert4SGeoA'
self.Message = 'AnnuOWL | Vert4SGeoA\nAUG_15_2023'
...
for i in range (0,4,1):
    ...
    anglePo=ang[3]
    angleMo=ang[4]
#    print(angle,angleP,angleM,anglePo,angleMo)
    p0=rs.CreatePoint(aa[0]+2.03*Rad*math.cos(angleMo),aa[1]+2.03*Rad*math.sin(angleMo),aa[2])
    p1=rs.CreatePoint(aa[0]+2.03*Rad*math.cos(angle),aa[1]+2.03*Rad*math.sin(angle),aa[2])
    p2=rs.CreatePoint(aa[0]+2.03*Rad*math.cos(anglePo),aa[1]+2.03*Rad*math.sin(anglePo),aa[2])
    p3=rs.CreatePoint(aa[0]+2.47*Rad*math.cos(angleM),aa[1]+2.47*Rad*math.sin(angleM),aa[2])
    p4=rs.CreatePoint(aa[0]+2.47*Rad*math.cos(angle),aa[1]+2.47*Rad*math.sin(angle),aa[2])
    p5=rs.CreatePoint(aa[0]+2.47*Rad*math.cos(angleP),aa[1]+2.47*Rad*math.sin(angleP),aa[2])
