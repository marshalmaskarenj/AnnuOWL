"""This component takes in the non-visual potential for each orientation, and calculates the color (material) for visualisation, depending upon the selected palette.
---
OWL (Occupant Well-being through Lighting) is developed by Marshal Maskarenj, for SCALE project funded by FNRS @ LAB, UCLouvain.
    Inputs:
        CS_N: The non-visual potential of each occupant-position, facing North (0 = Non Compliant, 1=Minimum, 2=Medium, 3=High)
        CS_E: The non-visual potential of each occupant-position, facing East (0 = Non Compliant, 1=Minimum, 2=Medium, 3=High)
        CS_S: The non-visual potential of each occupant-position, facing South (0 = Non Compliant, 1=Minimum, 2=Medium, 3=High)
        CS_W: The non-visual potential of each occupant-position, facing West (0 = Non Compliant, 1=Minimum, 2=Medium, 3=High)
    Output:
        CS_Mat: Combined materials for all orientations. Connects to Custom Preview"""
...
self.Name = "SCALE_Vert2SMat"
self.NickName = 'Vert2SMat'
self.Message = 'AnnuOWL | Vert2SMat\nAUG_15_2023'
...
if True:
    ...
    if CS_N:
        Mat_N,minN,maxN=illum2mat(CS_N,False,upperThres)
    else:
        Mat_N="No input found for Annual Circadian Stimulus Autonomy (N)"
    if CS_E:
        Mat_E,minE,maxE=illum2mat(CS_E,False,upperThres)
    else:
        Mat_E="No input found for Annual Circadian Stimulus Autonomy (E)"
    if CS_S:
        Mat_S,minS,maxS=illum2mat(CS_S,False,upperThres)
    else:
        Mat_S="No input found for Annual Circadian Stimulus Autonomy (S)"
    if CS_W:
        Mat_W,minW,maxW=illum2mat(CS_W,False,upperThres)
    else:
        Mat_W="No input found for Annual Circadian Stimulus Autonomy (W)"
    CS_min=str(minN)+","+str(minE)+","+str(minS)+","+str(minW)
    CS_max=str(maxN)+","+str(maxE)+","+str(maxS)+","+str(maxW)
#    print(minN,minE,minS,minW,maxN,maxE,maxS,maxW)
    CS_Mat=Mat_N + Mat_E + Mat_S + Mat_W
    print(CS_Mat)
